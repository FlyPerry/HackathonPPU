...
class Main:
    ...
    thread3 = threading.Thread(name="generateVehicles",target=generateVehicles, args=())    # Генерация авто
    thread3.daemon = True
    thread3.start()
    ...
    while True:
        ...
        screen.blit(background,(0,0))   # задний фон
        for i in range(0,noOfSignals):  # отображение текущего статуса светофора
            if(i==currentGreen):
                if(currentYellow==1):
                    if(signals[i].yellow==0):
                        signals[i].signalText = "Стоп!"
                    else:
                        signals[i].signalText = signals[i].yellow
                    screen.blit(yellowSignal, signalCoods[i])
                else:
                    if(signals[i].green==0):
                        signals[i].signalText = "Внимание!"
                    else:
        ...
        # отображаем таймер и количество
        for i in range(0,noOfSignals):  
        ...
        timeElapsedText = font.render(("Общее время (сек): "+str(timeElapsed)), True, black, white)
