...
options={
   'model':'./cfg/yolo.cfg',        #папка с конфигом
   'load':'./bin/yolov2.weights',   #weights
   'threshold':0.22        #Желательно ставить 0.3
}
...
def detectVehicles(filename):
   ...
   for vehicle in result:
      ...
      if(label=="car" or label=="bus" or label=="bike" or label=="truck" or label=="rickshaw"):    # Отрисовка лэйблов
         top_left=(vehicle['topleft']['x'],vehicle['topleft']['y'])
         bottom_right=(vehicle['bottomright']['x'],vehicle['bottomright']['y'])
         img=cv2.rectangle(img,top_left,bottom_right,(0,255,0),2)    #цвет бокса и размер
         img=cv2.putText(img,label,top_left,cv2.FONT_HERSHEY_DUPLEX,0.7,(0,0,0),1)   #image, label, position, font, font scale, colour: black, line width
